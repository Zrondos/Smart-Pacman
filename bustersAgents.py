    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        closestGhost = (None,inf)
        for distribution in livingGhostPositionDistributions:
            ghostLocation = distribution.argMax()
            distance = self.distancer.getDistance(ghostLocation,pacmanPosition)
            if distance < closestGhost[1]:
                closestGhost = (ghostLocation,distance)
        bestAction = (None,inf)
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(closestGhost[0],successorPosition)
            if distance < bestAction[1]:
                bestAction = (action,distance)
        return bestAction[0]