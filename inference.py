...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        toReturn = self.beliefs.copy()
        for key in toReturn.keys():
            toReturn[key] = 0
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos in self.allPositions:
                toReturn[newPos] += newPosDist[newPos]*self.beliefs[oldPos]
        self.beliefs = toReturn
        self.beliefs.normalize()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        self.legalStates = [state for state in itertools.product(self.legalPositions,repeat = self.numGhosts)]
        random.shuffle(self.legalStates)
        numLegalStates = len(self.legalStates)
        for i in range(self.numParticles):
            self.particles.append(self.legalStates[i%numLegalStates])
